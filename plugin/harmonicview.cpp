...
void HarmonicView::onVibecodeApiKeyJobFinished() {
  ...
  QKeychain::ReadPasswordJob *job = m_vibecodeApiKeyJob;
  QPointer<HarmonicView> viewGuard(this);

  if (job->error() == QKeychain::NoError) {
    m_vibecodeApiKey = job->textData();
  } else {
  ...
  // processing callbacks from old jobs if vibecode() was called multiple
  // times).
  if (viewGuard && viewGuard->m_vibecodeApiKeyJob == job) {
    m_vibecodeApiKeyJob->deleteLater();
